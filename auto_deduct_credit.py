...
def processing(csv_file_path, log_fn=print, should_stop_fn=None):
    ...
    try:
        ...
        time.sleep(2)
        # verify in right auction page, must use OCR
...
def auto_processing(bidcard_num: int, deduct_records: list[dict], log_fn=print) -> str:
    ...
    if all_deducted_count == 0 and has_partial_deduct:
        return [{'status': '2', 'details': "Partial credit has been deducted", 'row_offset': deduct_record['row_offset']} for deduct_record in deduct_records]

    if not IS_ONLINE:
        if has_partial_deduct:
            # Set the last one as partial and others as completed
            results = []
            for i, deduct_record in enumerate(deduct_records):
                if i == len(deduct_records) - 1:
                    # Last record - mark as partial
                    results.append({
                        'status': '2',
                        'details': "Partial credit has been deducted",
                        'row_offset': deduct_record['row_offset']
                    })
                else:
                    # Others - mark as completed
                    results.append({
                        'status': '1',
                        'details': "",
                        'row_offset': deduct_record['row_offset']
    ...
    # TODO split this method as two methods, one for processing credit deduction and the other for checking deduction result, so that we can have better error handling and avoid losing all results when there is error in checking result part 
        ...
        try:
            ...
            if cur_store_credit == last_credit:
                ...
                if same_credit_count >= 2:
                    raise MulStepError("Reached the end of credit list, stopping the check.")
            ...
            if not remaining_match:
                raise MulStepError(f"Unable to parse amount remaining for {cur_sc_invoice_number}, OCR result: {scetence}")
    ...
    # Append errors
    for deduct_record in deduct_records:
        if any(result['row_offset'] == deduct_record['row_offset'] for result in results):
            continue
        results.append({
            'status': '-1', 
            'details': f"Failed to verify credit deduction for invoice {deduct_record['sc_invoice_number']}, check errors for details.", 
            'row_offset': deduct_record['row_offset']
        })
    if errors and results:
        ...
        sorted_results = sorted(
            results,
            key=lambda result: int(result.get('row_offset', -1))
        )
        target_result = sorted_results[-1]
        for result in reversed(sorted_results):
            if str(result.get('status', '')).strip() == '-1':
                target_result = result
                break

        target_result['errors'] = error_text
